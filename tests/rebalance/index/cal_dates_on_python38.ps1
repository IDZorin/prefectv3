...
Start-Transcript -Path "logging\logging-caldate-${stamp}-${user}-powershell.log" 
...
function Run-Selection {
    ...
    write-output "`nadding requirements...`n"
    ...
    if (Test-Path -Path $currentscript -PathType Leaf) {
      write-output "$currentscript exists"
        ...
        write-output "ERROR $currentscript does not exist"
        pause
        exit 1
     }
    $currentscript= $selection_code + '\' + $selection_script_name
    ...
    python.exe $currentscript --repo_name $selection_name --calc_dates 1
